...
test('client bundle：ModuleLoader 工厂产出插件并注册皮肤设置与 PRTS 界面席位', async () => {
  ...
  const cssMin = cssSource.replace(/\s+/g, '')
  const source = clientSource + '\n' + cssSource
  assert.match(cssMin, /:not\(#prts-agent-scene\)/,
    '运行态必须读取 Conversation，不能读回场景自身')
  assert.match(cssMin, /--agent-content-center/,
  ...
  assert.match(cssMin, /--prts-agent-composer-y/,
    '输入卡必须支持独立的响应式垂直位移')
  assert.match(cssMin, /\[data-conversation-scroll\]>:not\(\[data-composer-seat\]\)\{background:transparent!important\}/,
  ...
  assert.match(cssMin, /\.prts-cpu-assembly\.is-purple/,
  ...
  assert.match(cssMin, /\[data-phase="active"\]:not\(#prts-agent-scene\)>:first-child/,
    '会话顶部栏样式必须排除 PRTS 场景，否则会把 560px 系统地图层染白')
  assert.match(cssMin, /\[data-chat-flow\]>\[data-chat-flow-key\]\{animation:prts-chat-history-enter/,
  ...
  assert.match(cssMin, />span:nth-child\(1\)>span:not\(\[data-state\]\).*clip-path:inset\(50%\)/s,
...
test('AIC skin：同步雪松林地图并规避 macOS 设置弹窗的 WebGL 合成卡顿', async () => {
  ...
  const skinCssMin = skinCss.replace(/\s+/g, '')
  ...
  assert.match(skinCssMin, /\[role="presentation"\]>\[aria-hidden="true"\].*backdrop-filter:none!important/s)
  assert.match(skinCssMin, /overflow:visible!important;pointer-events:none/)
