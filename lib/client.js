  ...
  factory: (require) => {
    ...
    const buildSceneSnapshotModel = (order, nodes) => {
      ...
      return {
        visuals,
        plan: {
          state: planState, title: '拆解问题意图', meta: stateLabel(planState, {
        ...
        verify: {
          state: verifyState, title: '交叉核验证据', meta: stateLabel(verifyState, {
    ...
    `
    /* ---- 皮肤样式：包内独立 .css（lib/skin/*.css），经 Host 路由
       /prts-corpus/skin 运行时加载后注入 <style>；不再内嵌模板字符串。---- */
    const SKIN_STYLE_BASE = '/prts-corpus/skin'
    let skinCssPromise = null
    const loadSkinStylesheets = () => {
      if (typeof fetch !== 'function') return Promise.resolve('')
      if (!skinCssPromise) {
        skinCssPromise = Promise.all([
          fetch(SKIN_STYLE_BASE + '/prts-agent.css', { cache: 'no-store' }),
          fetch(SKIN_STYLE_BASE + '/endfield-aic.css', { cache: 'no-store' }),
        ]).then(async (responses) => {
          if (!responses.every((r) => r.ok)) {
            throw new Error('skin css http ' + responses.map((r) => r.status).join('/'))
          }
          const parts = await Promise.all(responses.map((r) => r.text()))
          return parts[0] + '\n' + parts[1]
        }).catch((error) => { skinCssPromise = null; throw error })
      }
      return skinCssPromise
    }
    ...
    const buildEvidence = ({ byTitle, lastAnswer }) => {
      ...
      return [...used.values()].map((item) => {
        ...
        seen.add(signature)
        return {
          id: `${index}-${signature.slice(0, 24)}`, title: item.title, kind: 'story',
    ...
    async function readEvidenceSource(item) {
      ...
      const result = await callApi('read', {
        locator, selection: { mode: 'document' },
    ...
    function SourceReader({ item, onClose }) {
      ...
      return h('div', { className: 'prts-evidence-layer' },
        h('button', {
          type: 'button', className: 'prts-evidence-scrim', onClick: onClose,
          'aria-label': '关闭原文查看器'
        }),
        h('aside', {
          className: 'prts-source-reader', role: 'dialog', 'aria-modal': 'true',
          ...
          h('header', null,
            ...
            h('button', {
              type: 'button', className: 'prts-evidence-close', onClick: onClose,
              'aria-label': '关闭'
            }, '×')),
    ...
    function EvidenceControl({ useChat }) {
      ...
      return h(Fragment, null,
        h('button', {
          type: 'button', className: 'prts-header-badge', onClick: () => setOpen(true),
        ...
        open && !reader && h('div', { className: 'prts-evidence-layer' },
          h('button', {
            type: 'button', className: 'prts-evidence-scrim', onClick: () => setOpen(false),
            'aria-label': '关闭证据面板'
          }),
          h('aside', {
            className: 'prts-evidence-drawer', role: 'dialog', 'aria-modal': 'true',
            'aria-label': '本轮证据'
          },
            h('header', null,
              h('div', null, h('h3', null, '本轮证据'), h('p', null, 'LOCAL CORPUS / SOURCE READ TRACE')),
              h('button', {
                type: 'button', className: 'prts-evidence-close', onClick: () => setOpen(false),
                'aria-label': '关闭'
              }, '×')),
    ...
    const S = {
      ...
      badge: {
        padding: '1px 8px', borderRadius: '999px', fontSize: '11px', flex: '0 0 auto',
      ...
      input: {
        boxSizing: 'border-box', padding: '4px 8px', borderRadius: '6px',
      ...
      th: {
        textAlign: 'left', padding: '6px 8px', opacity: 0.6, fontWeight: 500,
      ...
      progressTrack: {
        height: '8px', borderRadius: '4px', background: 'rgba(128,128,128,.22)',
      ...
      buttonLink: {
        padding: '3px 0', border: 'none', background: 'transparent', color: 'inherit',
        ...
        h('div', { style: S.datasetGrid }, ...games.map((game) => {
          ...
          return h('section', {
            ...
            h('div', { style: { ...S.row, flexWrap: 'nowrap', alignItems: 'flex-start' } },
              h('input', {
                type: 'checkbox', checked: enabled, disabled: savingGames,
            ...
            h('div', { style: { ...S.row, justifyContent: 'space-between', marginTop: '8px' } },
              ...
              h('button', {
                style: S.buttonLink, disabled: !dataset?.present,
          ...
          ...versionRows.map((release) => {
            ...
            return h('div', { key: release.releaseId, style: S.versionRow },
              ...
              release.active
                ...
                : h(Fragment, null,
                  h('button', {
                    style: S.button, disabled: !release.complete,
                    onClick: () => onActivate(release.releaseId)
                  }, '切换'),
    ...
    function ConfigCard({ config, onSaved }) {
      ...
      return h('div', { style: S.card },
        ...
        h('div', { style: S.field },
          ...
          h('input', {
            type: 'checkbox', checked: Boolean(current('cloudEnabled', false)),
        ...
        h('div', { style: S.field },
          ...
          h('input', {
            style: S.input, value: String(current('cloudBaseUrl', '')),
        ...
        h('div', { style: S.grid2 },
          ...NUMBER_FIELDS.map(([key, label]) => h('div', { key, style: S.field },
            ...
            h('input', {
              style: { ...S.input, flex: '0 0 96px', width: '96px' }, type: 'number',
              value: String(current(key, '')), onChange: set(key, Number)
            })))),
        h('div', { style: S.field },
          h('span', { style: S.fieldKey }, '默认资料版本'),
          h('input', {
            style: S.input, value: String(current('downloadReleaseId', '')),
            onChange: set('downloadReleaseId')
          })),
        h('div', { style: S.field },
          h('span', { style: S.fieldKey }, '资料站点地址'),
          h('input', {
            style: S.input, value: String(current('downloadSiteBaseUrl', '')),
            onChange: set('downloadSiteBaseUrl')
          })),
        h('div', { style: S.field },
          h('span', { style: S.fieldKey }, '下载源顺序'),
          h('select', {
            style: S.select, value: current('downloadOrder', ['modelscope', 'site']).join(','),
      ...
      const checkUpdate = async () => {
        ...
        try {
          ...
          if (result.updateAvailable && result.remote) {
            setUpdateInfo({
              kind: 'update', remoteReleaseId: result.remote.releaseId,
    ...
    function AicMap({ focusPoint = .66 }) {
      ...
      useEffect(() => {
        ...
        const modalObserver = new MutationObserver(syncMapActivity)
      ...
      return h(Fragment, null,
        ...
        ...regions.map((region) => {
          ...
          return h('div', {
            key: region.lv, className: 'aic-region-label',
            style: {
              left: `${point.x}px`,
        ...
        h('button', {
          type: 'button', className: 'aic-map-reset', onClick: () => mapRef.current?.resetView(),
          title: '重置地图视角'
        }, 'RESET_VIEW'))
    }
    ...
    function AicRoot({ useSessions }) {
      ...
      // agent 运行态检测 → <html> 加 prts-running(呼吸) ；卸载清理
      useEffect(() => {
        if (typeof document === 'undefined') return () => {}
        const CLS = 'prts-running'
        let on = false
        const isRun = () => {
          for (const b of Array.from(document.querySelectorAll('button'))) {
            const a = (b.getAttribute('aria-label') || '')
            if (a.indexOf('停止生成') >= 0 || /^stop/i.test(a) || /stop generating/i.test(a)) return true
          }
          return false
        }
        const apply = () => { const v = isRun(); if (on !== v) { on = v; document.documentElement.classList.toggle(CLS, v) } }
        apply()
        const mo = new MutationObserver(() => apply())
        mo.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['aria-label'] })
        return () => { mo.disconnect(); document.documentElement.classList.remove(CLS) }
      }, [])
      ...
      const openSettings = () => {
        // 宿主触发按钮在 sidebar.settings 内的 .MI-_Aa_trigger（新结构下并非 >button 直接子节点），
        // 用 [aria-haspopup='dialog'] 定位（特性稳定），再兜底到普通 button。
        const trigger =
          document.querySelector('[data-slot="sidebar.settings"] [aria-haspopup="dialog"]')
          || document.querySelector('[data-slot="sidebar.settings"] button')
          || document.querySelector('[data-slot="sidebar.settings"]>button')
        if (trigger instanceof HTMLElement) trigger.click()
        else console.warn('[prts-terrarchive] AIC 设置入口未找到，无法打开宿主设置')
      }
      const startChatResize = (event) => {
        ...
        const hd = topHandleRef.current

        if (hd) hd.style.transition = 'none'

        const hn = topLineRef.current

        if (hn) hn.style.transition = 'none'

        let live = drawerRightRef.current

        const commit = () => { drawerRightRef.current = live; setChatBandWidth(live) }

        const resize = (me) => {

          const minW = Math.min(520, globalThis.innerWidth - 320)

          const maxW = Math.min(1200, globalThis.innerWidth - 320)

          live = Math.max(minW, Math.min(maxW, me.clientX))

          drawerRightRef.current = live

          document.body.style.setProperty('--prts-aic-band-width', `${live}px`)

          const d = topHandleRef.current

          if (d) d.style.left = `${live}px`

          dragYRef.current = Math.min(Math.max(me.clientY, 16), Math.max(16, globalThis.innerHeight - 16))

          if (d) d.style.top = `${dragYRef.current - 15}px`
        ...
        if (hd) hd.style.transition = ''

        if (hn) hn.style.transition = ''
      ...
      // 置顶拖宽把手：body 层 div，z 高于对话容器，可盖过对话文字抓取。

      // 拖动路线：只写 group 尺寸 CSS 变量与把手 style(不触发 React 重渲)，pointerup 才同步 setState，减卡顿。

      const drawerRightRef = useRef(chatBandWidth)

      drawerRightRef.current = chatBandWidth

      const chatActiveRef = useRef(chatResizeActive)

      chatActiveRef.current = chatResizeActive

      const topLineRef = useRef(null)
      const lineSet = (w, show) => { const ln = topLineRef.current; if (ln) { ln.style.left = `${w}px`; if (show) ln.style.opacity = '1'; else ln.style.opacity = '0' } }

      const topHandleRef = useRef(null)

      const setHandleStyle = (w, y) => { const d = topHandleRef.current; if (d) { d.style.left = `${w}px`; d.style.top = `${y - 15}px` } }

      const resizeNow = (w) => {

        drawerRightRef.current = w

        document.body.style.setProperty('--prts-aic-band-width', `${w}px`)

        setHandleStyle(w, dragYRef.current)

      }

      // 创建把手 DOM(钮+拖动黄线都放 body 层、内置可实时跟随)
      ...
      useEffect(() => {
        ...
        div.className = 'aic-chat-resize-top'

        document.body.appendChild(div)

        topHandleRef.current = div

        const line = document.createElement('div')

        line.className = 'aic-chat-resize-line'

        line.style.left = `${chatBandWidth}px`
        ...
        const onDown = (e) => { e.preventDefault(); stopChatResizeRef.current?.(); startChatResize(e) }

        div.addEventListener('pointerdown', onDown)

        return () => { div.removeEventListener('pointerdown', onDown); div.remove(); topHandleRef.current = null; line.remove(); topLineRef.current = null }

      }, [])

      // 跟随光标(靠近右缘)：黄线透明度随距离渐变(越近越实)，钮垂直跟随；拖动中由 lineSet 恒定 1。

      const dragYRef = useRef(112)
      ...
      useEffect(() => {
        ...
        const clampY = (y) => Math.min(Math.max(y, 16), Math.max(16, globalThis.innerHeight - 16))
        ...
        const apply = (x, y) => {
          ...
          const ln = topLineRef.current

          const right = drawerRightRef.current

          const act = chatActiveRef.current

          const dist = Math.abs(x - right)

          if (act) return

          const fade = dist >= FADE_RANGE ? 0 : 1 - dist / FADE_RANGE

          if (ln) ln.style.opacity = `${Math.max(0, Math.min(1, 1 - dist / FADE_RANGE)).toFixed(3)}`

          const targetY = dist <= 30 ? clampY(y) : 112

          dragYRef.current = targetY

          if (d) d.style.top = `${targetY - 15}px`
        ...
        return () => { if (raf) cancelAnimationFrame(raf); window.removeEventListener('pointermove', onMove) }
      ...
      return h('main', {
        ...
        h('section', { className: 'aic-chat-band' },
          ...
          h('div', { className: 'aic-terminal-overlay' },
            ...
            h('button', {
              type: 'button', className: 'aic-band-btn',
              onClick: () => {
                fetch('/api/desktop/developer/devtools', {
                  method: 'POST', headers: { 'content-type': 'application/json' },
                  body: '{}', cache: 'no-store',
                }).catch((err) => console.warn('[prts] devtools toggle', err))
              },
              title: '打开/关闭开发者工具（也可 F12）'
            }, '⟲ Dev'),
            h('button', {
              type: 'button', className: 'aic-band-btn aic-settings-trigger',
              onClick: openSettings
            }, '设置'),
            h('button', {
              type: 'button', className: `aic-band-btn${historyOpen ? ' on' : ''}`,
        ...
        historyOpen && h('aside', { className: 'aic-drawer' },
          ...
          h('div', { className: 'aic-session-list' }, sessionRows.length
            ? sessionRows.map((session) => h('button', {
    ...
    exports.apply = (ctx) => {
      // F12 → 打开/关闭宿主 DevTools（等价右上角“开发者选项”，便于增强模式内触发）
      ctx.effect(() => {
        ...
        const onKey = (e) => {
          if (e && e.key === 'F12') {
            ...
            fetch('/api/desktop/developer/devtools', {
              ...
              .catch((err) => console.warn('[prts] f12 devtools 失败', err))
          }
        }
      ...
      if (typeof document !== 'undefined') {
          ...
          loadSkinStylesheets().then((css) => {
            ...
            if (!disposed) console.error('[prts-terrarchive] skin stylesheet load failed', error)
          })
          return detach
