...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function AicRoot({ useSessions }) {
      ...
      // 左缘对齐抽屉右缘(chatBandWidth)、垂直停在 y；平时 y=112，靠近右缘时划到指针处。
      const [drawerHandleY, setDrawerHandleY] = useState(112)
      ...
      useEffect(() => {
        ...
        return () => { div.removeEventListener('pointerdown', onDown); div.remove(); topHandleRef.current = null }
      }, [])
      // 位置：右缘 + 垂直 y
      useEffect(() => {
        const div = topHandleRef.current
        if (!div) return
        div.style.left = `${chatBandWidth}px`
        div.style.top = `${drawerHandleY - 15}px`
      }, [chatBandWidth, drawerHandleY])
      // 靠近右缘 → 把手划到指针 y
      useEffect(() => {
        if (globalThis.innerWidth <= 980) return () => {}
        let raf = 0
        ...
        const onMove = (e) => {
          ...
          raf = requestAnimationFrame(() => {
            ...
            const near = Math.abs(e.clientX - right) <= 24
            if (act || near) setDrawerHandleY(clampY(e.clientY))
            else setDrawerHandleY(112)
          })
        }
      ...
      return h('main', {
        ...
        chatResizeActive && h('div', {
          className: 'aic-chat-resize-line',
