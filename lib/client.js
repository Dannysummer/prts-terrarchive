...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SCENE_HTML = `
      ...
      <div class="prts-trace-window prts-trace-query"><header><span>QUERY_DECOMPOSE</span><i></i></header><p><em>intent</em> agent request</p><p><em>scope</em> local / cloud</p><b><span></span> QUERY RESOLVED</b></div>
      <div class="prts-trace-window prts-trace-source"><header><span>SOURCE_READ</span><i></i></header><p><em>doc</em> PRTS LOCAL CORPUS</p><p><em>range</em> DOCUMENT / PAGED</p><b><span></span> SOURCE CONTEXT READY</b></div>
    ...
    const AIC_CSS = `
      ...
      .aic-chat-band{position:absolute;z-index:25;top:0;bottom:0;left:0;width:var(--aic-band-width);display:flex;flex-direction:column;pointer-events:none}
      ...
      @media(max-width:980px){.aic-chat-band{width:min(720px,100vw);padding-top:92px}.aic-hud-br{display:none}.aic-settings-layer{grid-template-columns:190px minmax(0,1fr)}.aic-settings-main{padding:24px 20px}.aic-brand-sub,.aic-coords span:nth-child(n+2){display:none}}
    ...
    const installScene = () => {
      ...
      const sync = () => {
        ...
        scene.dataset.running = search === 'active' || read === 'active' ? 'true' : 'false'
        ...
        if (tickerLabel) tickerLabel.textContent = read === 'active' ? '正在核验原文' : search === 'active' ? '正在检索候选' : phase === 'active' ? '本地证据链' : '本地检索'
        if (tickerState) tickerState.textContent = scene.dataset.running === 'true' ? 'LIVE' : phase === 'active' ? 'DONE' : 'IDLE'
      }
...
.prts-source-reader{position:fixed;z-index:81;top:0;right:0;bottom:0;width:min(560px,94vw);padding:22px;overflow:auto;color:#111214;background:rgba(249,250,247,.97);border-left:1px solid rgba(17,18,20,.12);box-shadow:-28px 0 70px rgba(17,18,20,.16);backdrop-filter:blur(22px);animation:prts-drawer-in .24s cubic-bezier(.2,.72,.2,1)}.prts-source-reader>header{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;padding-bottom:17px;border-bottom:1px solid rgba(17,18,20,.12)}.prts-source-reader h3{margin:0;font-size:18px;line-height:26px;word-break:break-word}.prts-source-reader header p{margin:3px 0 0;color:#858a91;font:8px/13px ui-monospace,monospace;letter-spacing:.08em}.prts-source-body{padding-top:14px}.prts-source-state{padding:52px 20px;color:#858a91;text-align:center;font-size:12px;line-height:20px}.prts-source-error{color:#b64444}.prts-source-line{display:flex;gap:9px;padding:3px 2px;line-height:1.7;font-size:12px;color:#34373b}.prts-source-line .prts-source-no{flex:none;width:30px;color:#9aa0a6;font:9px/1.7 ui-monospace,monospace;text-align:right}.prts-source-line .prts-source-speaker{flex:none;color:#8a5800;font-weight:500}.prts-source-line.narration{color:#5a5f66}.prts-source-line.active{background:rgba(40,105,216,.12);box-shadow:inset 2px 0 0 var(--agent-blue,#2869d8)}
      ...
      body[data-prts-skin="agent"] div[role="treeitem"][aria-selected]:has(>span:nth-child(4)){position:relative;display:grid!important;grid-template-columns:18px minmax(0,1fr)!important;grid-template-rows:20px 17px!important;column-gap:10px!important;row-gap:0!important;height:54px!important;margin:3px 0!important;padding:7px 10px!important;border-radius:12px!important}body[data-prts-skin="agent"] div[role="treeitem"][aria-selected]:has(>span:nth-child(4))>span:nth-child(1){grid-column:1;grid-row:1/3;align-self:center;width:18px!important}body[data-prts-skin="agent"] div[role="treeitem"][aria-selected]:has(>span:nth-child(4))>span:nth-child(2){grid-column:2;grid-row:1;align-self:end;min-width:0;margin:0!important;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:14px;line-height:20px;font-weight:600}body[data-prts-skin="agent"] div[role="treeitem"][aria-selected]:has(>span:nth-child(4))>span:nth-child(3){grid-column:2;grid-row:2;align-self:start;font-size:11px;line-height:17px}body[data-prts-skin="agent"] div[role="treeitem"][aria-selected]:has(>span:nth-child(4))>span:nth-child(4){position:absolute;top:17px;right:9px}
    ...
    /* ---- 资料库状态卡片（对齐前端「本地资料」页） ---- */
    function StatusCard({ status, activeRelease, onRefresh, onCheckUpdate, checking, updateInfo }) {
      ...
      const documentCount = activeRelease?.documentCount ?? store.documentCount ?? null
      const sizeBytes = activeRelease?.sizeBytes ?? null
      return h('div', { style: S.card },
        h('div', { style: { ...S.row, justifyContent: 'space-between' } },
          h('h3', { style: S.cardTitle }, '资料库状态'),
          h('button', { style: S.button, disabled: checking, onClick: onCheckUpdate },
            checking ? '检查中…' : '检查版本')),
        h('div', { style: S.row },
          h('span', { style: S.badge }, '文档'),
          h('strong', null, documentCount != null ? `${documentCount} 篇` : '—'),
          h('span', { style: S.hint },
            store.releaseId ? `版本 ${store.releaseId}` : '（未加载）'),
          store.loaded ? h('span', { style: S.badgeGreen }, '已就绪') : h('span', { style: S.badgeGray }, '加载中/未安装')),
        h('div', { style: S.row },
          h('span', { style: S.key }, '本地大小'),
          h('code', { style: S.val }, fmtBytes(sizeBytes)),
          h('span', { style: S.hint }, `数据版本 ${shortVersion(activeRelease?.dataVersion ?? store.dataVersion)}`)),
        updateInfo?.text && h('div', { style: { ...S.row, marginTop: '8px' } },
          ...
          updateInfo.kind === 'update' && updateInfo.remoteReleaseId
            && h('button', { style: S.buttonPrimary, onClick: updateInfo.onDownload }, '下载新版')),
    ...
    /* ---- 版本管理（前端风格版本列表） ---- */
    function ReleasesCard({ releases, activeId, onActivate, onDelete, onDownload, downloading }) {
      const rows = (releases ?? []).map((item) => h('tr', { key: item.releaseId },
        h('td', { style: { ...S.td, ...S.val } }, item.releaseId),
        h('td', { style: { ...S.td, whiteSpace: 'nowrap' } }, item.documentCount ?? '—'),
        h('td', { style: { ...S.td, whiteSpace: 'nowrap' } }, fmtBytes(item.sizeBytes)),
        h('td', { style: { ...S.td, whiteSpace: 'nowrap', opacity: 0.6 } }, fmtDate(item.createdAt)),
        h('td', { style: { ...S.td, whiteSpace: 'nowrap' } }, item.active
          ? h('span', { style: S.badgeGreen }, '当前')
          : item.complete ? h('span', { style: S.badgeGray }, '完整')
            : h('span', { style: S.badgeRed }, '不完整')),
        h('td', { style: { ...S.td, whiteSpace: 'nowrap' } }, h('div', { style: { ...S.row, flexWrap: 'nowrap' } },
          item.active ? null : h('button', { style: S.button, disabled: !item.complete,
            onClick: () => { onActivate(item.releaseId) } }, '激活'),
          item.active ? null : h('button', { style: S.button,
            onClick: () => { onDelete(item.releaseId) } }, '删除')))))
      const emptyRow = h('tr', { key: 'empty' },
        h('td', { style: S.td, colSpan: 6 },
          h('span', { style: S.hint }, '本地暂无资料版本——点上方按钮下载')))
      const submitCustom = (event) => {
        const input = event.target.previousElementSibling
        const value = input ? input.value.trim() : ''
        if (value) { onDownload(value); input.value = '' }
      }
      return h('div', { style: S.card },
        h('h3', { style: S.cardTitle }, '版本管理'),
        h('div', { style: { ...S.row, marginBottom: '10px' } },
          h('button', { style: S.buttonPrimary, disabled: downloading, onClick: onDownload },
            downloading ? '下载中…' : '下载最新资料'),
          h('input', { style: S.input, placeholder: 'releaseId（可手输）', defaultValue: '' }),
          h('button', { style: S.button, disabled: downloading, onClick: submitCustom }, '下载指定版本')),
        h('table', { style: S.table },
          h('thead', null, h('tr', null,
            h('th', { style: { ...S.th, width: '30%' } }, '版本'),
            h('th', { style: { ...S.th, width: '8%' } }, '文档'),
            h('th', { style: { ...S.th, width: '14%' } }, '大小'),
            h('th', { style: { ...S.th, width: '12%' } }, '发布'),
            h('th', { style: { ...S.th, width: '12%' } }, '状态'),
            h('th', { style: { ...S.th, width: '24%' } }, '操作'))),
          h('tbody', null, rows.length === 0 ? emptyRow : rows)),
        h('div', { style: { ...S.hint, marginTop: '6px' } },
          '下载与版本检查优先走 ModelScope 镜像；本站仅在 ModelScope 不可达时回退。切换激活后索引即时重建，无需重启。'))
    }
    ...
    function ConfigCard({ config, onSaved }) {
      ...
      return h('div', { style: S.card },
        ...
        h('div', { style: S.field },
          h('span', { style: S.fieldKey }, '游戏命名空间'),
          h('select', { style: S.select, value: String(current('cloudGame', 'arknights')),
              onChange: set('cloudGame') },
            h('option', { value: 'arknights' }, 'arknights')),
          h('span', { style: { ...S.hint, flexBasis: '100%', marginLeft: '148px' } }, '终末地接入中，暂不可选')),
        h('div', { style: S.grid2 },
    ...
    function PrtsSection() {
      ...
      return h('div', { style: S.wrap, className: 'prts-settings-wrap' },
        ...
        h(StatusCard, { status, activeRelease, onRefresh: refresh,
          onCheckUpdate: checkUpdate, checking, updateInfo: statusUpdateInfo }),
        h(ReleasesCard, {
          releases: releaseList?.releases ?? [], activeId: releaseList?.activeId,
    ...
    function AicMap() {
      ...
      useEffect(() => {
        ...
        void loadMapBundle().then(async (runtime) => {
          ...
          else { mapRef.current = map; setStatus(''); aicBootDone() }
        }).catch((error) => {
          if (!disposed) setStatus(`ERROR // ${error.message}`)
          aicBootProgress(-1, error.message)
          setTimeout(() => { if (aicBootEl) aicBootDone() }, 2500)
        })
        return () => { disposed = true; mapRef.current?.dispose(); mapRef.current = null }
      }, [])
      ...
      return h(Fragment, null,
        ...
        h('div', { className: 'aic-hud aic-hud-tl' },
          ...
          h('div', { className: 'aic-coords' }, h('span', null, `VIEW ${stats.cx},${stats.cy}`),
            ...
            h('span', null, `${stats.fps || '--'}FPS · ${stats.meshes || '--'}MESH · ${stats.tris ? Math.round(stats.tris / 1000) + 'K' : '--'}TRI`))),
    ...
    function AicRoot({ useSessions }) {
      ...
      return h('main', { className: 'aic-root' },
        h(AicMap),
        h('section', { className: 'aic-chat-band' },
          ...
          h('div', { className: 'aic-terminal-overlay' },
            ...
            h('button', { type: 'button', className: 'aic-band-btn aic-settings-trigger',
              onClick: () => document.querySelector('[data-slot="sidebar.settings"]>button')?.click() }, '设置'),
